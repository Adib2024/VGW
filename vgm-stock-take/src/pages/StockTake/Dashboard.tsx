  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f0f2f5', padding: '1rem', fontFamily: 'Inter, sans-serif', paddingBottom: '3rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1rem', maxWidth: '800px', margin: '0 auto' }}>
        ...
        {[
          { key: 'b17', title: 'LOCATION B17' },
          { key: 'b22', title: 'LOCATION B22', bg: '#ffebee' },
          { key: 'loma', title: 'LOMA', bg: '#e8f5e9' },
          { key: 'b22_seq', title: 'B22 SEQ', bg: '#fff8e1' }
        ].map((zone) => {
            ...
            <Card key={zone.key} onClick={() => handleCardClick()} className="dashboard-card" style={{ cursor: 'pointer', padding: '1.5rem', borderRadius: '16px', backgroundColor: '#fff', boxShadow: '0 4px 15px rgba(0,0,0,0.03)', transition: 'transform 0.2s' }}>
              <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem' }}>
                <div style={{ width: '48px', height: '48px', borderRadius: '50%', backgroundColor: zone.bg || '#e3f2fd', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <img src="https://upload.wikimedia.org/wikipedia/commons/6/6d/Volkswagen_logo_2019.svg" alt="VW" style={{ width: '28px', height: '28px' }} />
                </div>
                <div>
                  <h2 style={{ margin: '0 0 0.25rem 0', fontSize: '1.1rem', color: '#001e50', fontWeight: 700 }}>{zone.title}</h2>
                  <div style={{ color: '#666', fontSize: '0.875rem' }}>{s.completed} / {s.total} Items</div>
                </div>
              </div>
              
              <ProgressBar percentage={s.percentage} showCar={true} label={s.percentage === 100 ? "Ready" : "Pending"} />
            </Card>
          );
        })}
      </div>
      ...
      {showLogoutConfirm && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ backgroundColor: '#fff', padding: '2rem', borderRadius: '16px', maxWidth: '320px', width: '90%', textAlign: 'center' }}>
            <h3 style={{ margin: '0 0 1rem 0', color: '#001e50' }}>Logout</h3>
            <p style={{ margin: '0 0 2rem 0', color: '#666' }}>Are you sure you want to log out?</p>
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
              <button onClick={() => setShowLogoutConfirm(false)} style={{ padding: '0.5rem 1.5rem', borderRadius: '8px', border: '1px solid #ccc', background: '#fff', cursor: 'pointer', fontWeight: 600 }}>Cancel</button>
              <button onClick={handleLogout} style={{ padding: '0.5rem 1.5rem', borderRadius: '8px', border: 'none', background: 'red', color: '#fff', cursor: 'pointer', fontWeight: 600 }}>Logout</button>
            </div>
          </div>
        </div>
      )}
